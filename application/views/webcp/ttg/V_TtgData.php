<?php if($ttg){ ?>
      ...
      <div onclick="openDetailNews('<?=$n['id']?>')" class="col-lg-4 col-md-12 news-data">
        <article class="entry news-data-body">
          ...
          <h2 class="entry-title judul-berita">
            <a href="<?=base_url('ttg/detail/'.$n['id'])?>" title="<?=$n['judul']?>" class="header_news"><?=$n['judul']?></a>
          </h2>
          ...
          <div class="entry-content">
            ...
            <div class="read-more mt-2">
              <a href="<?=base_url('ttg/detail/'.$n['id'])?>">Selengkapnya</a>
            </div>
          </div>
  ...
  <script>
    ...
    function openDetailNews(id){
      location.href= "<?=base_url('ttg/detail')?>"+'/'+id;
    }
