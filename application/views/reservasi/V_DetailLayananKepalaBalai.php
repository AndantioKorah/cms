        ...
        <?php foreach($rs['parameter'] as $p){ if($p['checked'] == 1){ ?>
          <tr style="border: 1px solid black;">
            ...
            <td style="padding: 5px; width: 30%;">
              <input autocomplete="off" disabled class="form-control form-control-sm" name="hasil_<?=$p['id_t_reservasi_online_parameter']?>" value="<?=$p['hasil_lab']?>" />
            </td>
            <td style="padding: 5px; width: 30%;">
              <input autocomplete="off" disabled class="form-control form-control-sm" name="catatan_<?=$p['id_t_reservasi_online_parameter']?>" value="<?=$p['catatan_lab']?>"/>
            </td>
          </tr>
        <?php } }?>
      <?php } ?>
